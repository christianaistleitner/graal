...
public final class GCImpl implements GC {
    ...
    private static void verbosePostCondition() {
        ...
        if (runtimeAssertions() || forceForTesting) {
            ...
            if (forceForTesting) {
                ...
                /* Print raw fields before trying to walk the chunk lists. */
                log.string("  These should all be 0:").newline();
                log.string("    oldGen toSpace first AlignedChunk:   ").zhex(oldGen.getSpace().getFirstAlignedHeapChunk()).newline();
                log.string("    oldGen toSpace last  AlignedChunk:   ").zhex(oldGen.getSpace().getLastAlignedHeapChunk()).newline();
                log.string("    oldGen.toSpace first UnalignedChunk: ").zhex(oldGen.getSpace().getFirstUnalignedHeapChunk()).newline();
                log.string("    oldGen.toSpace last  UnalignedChunk: ").zhex(oldGen.getSpace().getLastUnalignedHeapChunk()).newline();
    ...
    Object promoteObject(Object original, UnsignedWord header) {
        ...
        if (result == null) { // complete collection, tenuring age reached, or survivor space full
            ...
            //Log.log().character('[').object(result).string(" moved in old gen]").newline().flush();
